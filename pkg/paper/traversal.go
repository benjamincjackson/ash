...
func LabelChangesSynNonsyn(t *tree.Tree, regions []annotation.Region, characters []characterio.CharacterStruct, states [][]byte) {
	labelChangesSynNonsyn(t.Root(), nil, regions, characters, states)
}

// recursive function to label branches with state changes
func labelChangesSynNonsyn(cur, prev *tree.Node, regions []annotation.Region, characters []characterio.CharacterStruct, states [][]byte) {
	...
	for i, n := range cur.Neigh() {
		if n != prev {
			...
			labelEdgeSynNonsyn(cur, n, cur.Edges()[i], regions, characters, states)
			// and then we carry on recurring down the tree
			labelChangesSynNonsyn(n, cur, regions, characters, states)
		}
	}
}

// label an edge with annotated changes - amino acid changing versus neutral nucleotide change
func labelEdgeSynNonsyn(upnode, downnode *tree.Node, edge *tree.Edge, regions []annotation.Region, characters []characterio.CharacterStruct, states [][]byte) {
	up_id := upnode.Id()
	down_id := downnode.Id()
			...
			for _, codonstart := range region.Codonstarts {
				...
				if bitsets.Different(states[up_id][codonstart-1:codonstart+2], states[down_id][codonstart-1:codonstart+2]) {
					...
					case true:
						...
						// They are different. if there is only one nuc change, we label it
						case true:
							if len(nuclabels) == 1 {
								label := "nonSyn=" + nuclabels[0]
								edge.AddComment(label)
							}

						// They are the same. if there is only one nuc change, we label it
						case false:
							if len(nuclabels) == 1 {
								label := "syn=" + nuclabels[0]
								edge.AddComment(label)
							}
						}

					// if we can't, then we do nothing
					case false:
					}
				}
